...
class Solution
  ...
  def racer_names
    return @coll.find.aggregate([{:$project=>{ :_id=>0, :first_name=>1,:last_name=>1 }}])
  end

  def id_number_map
    return @coll.find.aggregate([{:$project=>{ :_id=>1, :number=>1 }}])
  end

  def concat_names
    return @coll.find.aggregate([{:$project=>{ :_id=>0, :number=>1, :name=>{ :$concat=>[ "$first_name"," , ","$last_name" ]}}}])
  end
  ...
  def group_times
    #@coll.find.aggregate([{:$group=>{:_id=>{:gender=>'$gender',:group=>'$group'},count:{:$sum => 1}, :fastest_time=>{:$min=>'$secs'}}}])
    @coll.find.aggregate([{:$group=>{:_id=>{:gender=>'$gender',:age=>'$group'}, runners:{:$sum => 1}, fastest_time:{:$min=>'$secs'}}}])
  end

  def group_last_names
    @coll.find.aggregate([{:$group=>{:_id=>{:gender=>'$gender',:age=>'$group'}, last_names:{:$push=>'$_id'}}}])
  end
  ...
  def groups_faster_than criteria_time
    @coll.find.aggregate([{:$group=>{:_id=>{:gender=>'$gender',:age=>'$group'}, runners:{:$sum => 1}, fastest_time:{:$min=>'$secs'}}}, {:$match=>{fastest_time:{:$lte=> criteria_time}}}])
  end

  def age_groups_faster_than age_group, criteria_time
    @coll.find.aggregate([{:$match=>{group:{:$eq => age_group}}},{:$group=>{:_id=>{:gender=>'$gender',:age=>'$group'}, runners:{:$sum => 1}, fastest_time:{:$min=>'$secs'}}}, {:$match=>{fastest_time:{:$lte=> criteria_time}}}])
  end
  ...
  def avg_family_time last_name
    @coll.find.aggregate([{:$match=>{last_name:{:$eq => last_name}}},{:$group=>{:_id=>'$last_name', :avg_time=>{:$avg=>'$secs'}, :numbers=>{:$push=>'$number'} }}])
  end

  def number_goal last_name
    @coll.find.aggregate([{:$match=>{last_name:{:$eq => last_name}}},{:$group=>{:_id=>'$last_name', :avg_time=>{:$avg=>'$secs'}, :numbers=>{:$push=>'$number'} }},{:$unwind=>'$numbers'}, {:$project=>{ :_id=>0, :last_name=>'$_id',:number=>'$numbers',:avg_time=>1 }}])
  end
